...
plot(x$Area_m2, x$Inflow_m3_yr, pch = 16, log = "xy",
     xlab = "Area (m2)", ylab = "Inflow (m3/yr)")

plot(x$Area_m2, x$TP_Inflow_mg_L, pch = 16, log = "xy",
     xlab = "Area (m2)", ylab = "Inlet [TP] (mg/L)")

plot(x$Inflow_m3_yr, x$TP_Inflow_mg_L, pch = 16, log = "xy",
     xlab = "Inflow (m3/yr)", ylab = "Inlet [TP] (mg/L)",
     col = c("#a1a1a1bb", "#bd4ad4bb","#e34327bb", "#345bebbb", "#2b821fbb")[x$Water_regime])
...
plot(x$HLR, x$TP_Inflow_mg_L, pch = 16, log = "xy",
     xlab = "HLR", ylab = "Inlet [TP] (mg/L)")
     #col = c("#a1a1a1bb", "#bd4ad4bb","#e34327bb", "#345bebbb", "#2b821fbb")[x$Water_regime])

plot(x$Age_yr, x$Area_m2, pch = 16, log = "xy",
     xlab = "Age (years)", ylab = "Area (m2)")
...
d <- x %>%
  select(Catchment_Type, Wetland_Type, Water_regime,
         Long, Age_yr, Area_m2, Depth_m, Inflow_m3_yr, HRT_d, HLR,
         TP_Inflow_mg_L, TP_load_in_g_m2_yr, SRP_Inflow_mg_L, SRP_load_in_g_m2_yr )

d <- x %>%
  select(Long, Age_yr, Area_m2, Depth_m, Inflow_m3_yr, HRT_d, HLR,
         TP_Inflow_mg_L, TP_load_in_g_m2_yr, SRP_Inflow_mg_L, SRP_load_in_g_m2_yr )

cor_scores <- cor(d[,c( "Long",     "Age_yr"  , "Area_m2" ,    "Depth_m"  , "Inflow_m3_yr" ,
                        "HRT_d", "HLR" ,"TP_Inflow_mg_L" , "TP_load_in_g_m2_yr",  "SRP_Inflow_mg_L" ,"SRP_load_in_g_m2_yr")])
...
plot(x$logArea_m2, x$logInflow_m3_yr, pch = 16,
     col = c("#a1a1a1bb", "#bd4ad4bb","#e34327bb", "#345bebbb", "#2b821fbb")[x$Water_regime])
legend("bottomright", levels(x$Water_regime), 
       pch = 16, pt.cex = 1.5,
       ncol = 2,
