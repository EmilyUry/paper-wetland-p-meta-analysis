...
c <- ggplot(data, aes(x = factor(species, level = c("TP", "SRP")), y = (num), fill = behavior)) +
  ...
  geom_text(aes(y = label_ypos, label = label_text), vjust = 0, hjust = "middle", color = "white", size = 2.5)+
  theme_classic() +
...
d <- ggplot(dd, aes(Source, factor(Flow, level = c("n.s.", "continuous, constant", "continuous, variable", 
  ...
  theme_classic() +
  scale_y_discrete(labels = c( "n.s.", "cont., const.", "cont., var",  "int.,  const.", "int., var.")) +
...
e <- rbind(ag, urb, WTP) %>%
  mutate(type = fct_relevel(type, "WTP", "Urban", "Ag"))
